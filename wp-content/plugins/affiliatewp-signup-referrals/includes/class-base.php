...
class AffiliateWP_Signup_Referrals_Base {
	...
	public function ultimate_member_user_signup( $user_id, $args ) {
		...
		$this->add_referral( $args );
		do_action( 'affwp_insert_new_referral', $user_id );//Added by Manjunath : created hook for Postback tracking.
	}
	...
	public function affiliate_approved( $affiliate_id, $status, $old_status ) {
        ...
        switch ( $context ) {
			...
			case 'ultimate_member':
				$description = __( 'Masala Landing Form', 'affiliatewp-signup-referrals' );
			break;
		...
		public function add_referral( $args = array() ) {
			...
			// Assign default affiliate, if no referring affiliate ID - Esaikkidoss - 27/02/2020
			if ( ! $referring_affiliate_id ) {
				$referring_affiliate_id = 20; // 20-Masalaforshape
			}
			...
			if ( affwp_is_active_affiliate( $referring_affiliate_id ) ) {
				//$campaign_id=(($_GET['campaign'] == '') ? NULL : $_GET['campaign']);  //Added by Keerthi
				...
				$args = apply_filters( 'affwp_signup_referrals_args', array(
					...
					'reference'    => $reference,
					'type'    	   => 'lead', //Added by Keerthi
					'campaign'    	   => $_GET['campaign'], //Added by Keerthi
